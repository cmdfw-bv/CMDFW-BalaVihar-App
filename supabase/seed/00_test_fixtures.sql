-- pgTAP test-fixture surface — the `tests` schema, its helpers, and the pgtap extension.
--
-- THIS FILE MUST NOT BECOME A MIGRATION. ADR-2026-09-07-test-fixtures-never-in-migrations,
-- Decision 1: test-only database objects install via the seed path, never a migration.
-- `tests.create_supabase_user()` is SECURITY DEFINER and writes directly into auth.users —
-- an account factory. Migrations apply to every environment including production; seed files
-- load only during `supabase db reset`. That difference is the whole point of this file's
-- location, and scripts/check-migration-fixtures.js fails CI if it is moved back.
--
-- Declared ahead of seed.sql in config.toml's [db.seed] sql_paths, which the CLI processes in
-- declared order: seed.sql calls create_supabase_user() at 8 sites.
--
-- Body below is 20260709022932_enable_pgtap_and_test_helpers.sql, relocated unchanged.
-- ---------------------------------------------------------------------------------------
