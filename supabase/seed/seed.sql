...
begin
  ...
  for v_class in select id, grade_band from classes where session_id = v_f3 order by grade_band loop
    i := i + 1;
  ...
  -- Attendance + class_updates for each running class's last 4 completed scheduled meetings,
  -- with a deliberate compliant / partial / non-compliant mix for the compliance dashboard demo
  -- (derived from class_meetings, not a hardcoded calendar):
  --   * class at position 2 -> no attendance      (non-compliant on attendance)
  --   * class at position % 3 = 0 -> no updates    (non-compliant on updates)
  --   * the rest -> fully compliant
  for v_class in
  ...
  loop
    ...
    loop
      if v_pos <> 2 then
        ...
        select e.id, d,
               case when (v_pos + extract(day from d)::int) % 5 = 0 then 'absent' else 'present' end,
               v_teacher_user_id
  ...
  insert into family_members (family_id, user_id, relationship)
    values ((select id from families order by label limit 1), v_multirole_user_id, 'guardian');
