...
- **Five jobs at POC:** `push-send` (web-push), `email-send` (AWS SES), `csv-import` (enrollment, service role), `user-role-sweep` (auto-activation sweep, service role), `user-role-grant` (tiered manual role grant/revoke, service role — ADR-0026). Don't add functions that duplicate what the client can do directly against RLS-protected Supabase.
