...
// failed fetch.
//
// PreToolUse on Bash. The read-modify-write round trip used to update an issue or PR body —
//   gh issue view N --json body --jq .body > f   &&   <edit f>   &&   gh issue edit N --body-file f
// — destroys the remote body when the READ fails. `gh` writes its error to the file, the edit
// happily pushes it, and the original text is gone with no error anywhere in the chain.
//
  ...
  try {
    ...
    for (const f of files) {
      ...
      try {
        ...
        bad.push([f, "the file does not exist or cannot be read"]);
        continue;
      }
