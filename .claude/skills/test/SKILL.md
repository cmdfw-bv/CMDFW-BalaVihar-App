...
3b. **Design parity gate — mandatory whenever the diff touches `app/**`, `components/**`, or `features/**`** (a changed component counts even if the only diff is in a shared component consumed elsewhere — check its consumers too). Run a **design review** with playwright-cli at **both** mobile-web breakpoints (360/768) **and** desktop-web breakpoints (1024/1440) for every affected screen:
   - Render each breakpoint and compare against the matching Open Design reference (`bv-connect/screens/{desktop,phone}` via `DesignSync`, or local `design/sankalp/prototypes/<screen>.html`) + the DoD (`design-system` skill).
   - Explicitly check **web/mobile-web parity** — flag anything that renders correctly at one set of breakpoints but not the other (missed responsive case, a fix applied on one side only, divergent copy/state/layout).
   - Skip only when the diff has zero FE-component impact (e.g., pure backend/migration/doc change) — state that explicitly rather than omitting the step silently.
