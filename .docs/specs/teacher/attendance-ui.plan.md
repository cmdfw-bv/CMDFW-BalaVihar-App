...
> Spec: [attendance-ui.md](attendance-ui.md) · ADR-0019, ADR-0021, ADR-0030, ADR-0031 (all Closed, unchanged) · stage: `/plan` sync ✓ → `/migration` ✓ → `/build` ✓ (2026-07-24) → PR #49 code review: F12 correction-flow fix (2026-07-25) → `/test` → `/deploy-staging` → `/promote`
>
...
>
> **Correction-flow fix (2026-07-25, PR #49 code review, `attendance-ui.md` decision #6, human-signed-off):** F12 below is revised — `SegmentedTabs` is no longer swapped out for `StatusChip` once `mark.saved`; it stays rendered and interactive always, with `StatusChip` shown alongside it as a confirmation marker. No other stage changes: `useAttendanceRoster.ts` (F11)'s public shape and `markLocal` behavior are unchanged (it already reset `saved: false` on any tap), so this is JSX-only, verified by hand per the TDD-boundary note below, not a RED→GREEN unit-test cycle.
...
- [x] **F12 — `lib/attendance/AttendanceRosterRow.tsx`** (revised 2026-07-25, correction-flow fix): one `Card` (`tone="surface"`) per roster row — student `firstName lastName · gradeLevel`; `SegmentedTabs` (`tabs={['present','absent']}`, `value={mark.status}`, `onChange={(id) => markLocal(row.enrollmentId, id as 'present'|'absent')}`) is **always rendered and always interactive**, saved or not — this is what makes correcting an already-submitted row (AC#2) reachable, since `markLocal` already resets `saved: false`/`error: false` on any tap regardless of the row's current `saved` value. When `mark.saved`, a `StatusChip status={mark.status}` renders **alongside** `SegmentedTabs` (not in its place) as a passive, read-only confirmation marker — tapping a segment on a saved row makes the chip disappear immediately (local state flips to unsaved) until the row is resubmitted. When `mark.error`, no `StatusChip` renders (the row was never `saved: true` on this pass) and an inline `--danger`-toned note appears instead ("Couldn't save — retry"), reusing `Field`'s reserved-hint-slot visual language (`theme.colors.status.absent`, per `Field.tsx`'s `hint` style) rather than inventing a new error-text style.
