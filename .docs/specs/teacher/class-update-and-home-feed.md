...
Comment moderation, class roster screen, announcements, structured homework/assignment tracking, and the `push-send` function's shared phases (owned by `notifications-infra`) — all as stated in the refine brief above. **Added by this pass:** a Coordinator/BV Coordinator/Admin oversight *screen* for browsing class updates/comments — this item grants the DB-level read (ADR-0030, extended by decision #2) but builds no UI consumer of it for these roles; that's a future item's decision (the Teacher/Student/Parent UI in this item's own scope is unaffected).
