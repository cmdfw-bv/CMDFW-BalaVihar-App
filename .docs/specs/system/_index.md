...
| [user-role-approval](user-role-approval.md) | POC-core (doc 2: Coordinator/BV Coordinator/Admin approval flows) | System | Admin, Coordinator, BV Coordinator | privileged role/scope-assignment function (writes `user_roles`) — auto-sweep for CSV-provisioned parents/students + tiered manual grant/revoke for teacher/coordinator/bv_coordinator/admin | Built — review-clean, full suite green (149/149 unit + 130/130 pgTAP); two real access-control bugs (revoke-by-id trusting request-body role/scope over the resolved row) found + fixed + adversarially re-verified during review; live local-stack smoke pass done; ready for `/test` ([plan](user-role-approval.plan.md)) |
