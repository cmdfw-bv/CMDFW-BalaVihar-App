...
> 2. **Secret scanning: gitleaks CLI binary directly, not the `gitleaks-action` wrapper.** The wrapper's PR-annotation features require a paid `GITLEAKS_LICENSE` for GitHub-Organization-owned repos (this repo, `mehtamaulik-creator/CMDFW-BalaVihar---Pilot-App`, is a personal-account repo per `gh repo view`, so the wrapper's free tier would apply today — but pinning to the raw CLI avoids the license question ever becoming load-bearing if repo ownership ever moves to an org). The open-source `gitleaks` CLI itself (MIT-licensed) has no such gate; running it directly keeps ADR-0024's "no CI secrets expected" consequence true.
