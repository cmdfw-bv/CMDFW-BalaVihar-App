...
- [x] **F24 — `lib/isDesktopWidth.ts`** (pure, TDD) — `(width: number) => boolean`, `width >= breakpoints.lg`. `lib/useIsDesktop.ts` is a thin wrapper over RN's `useWindowDimensions`, same untested-wiring seam as Stage 8's `useRoleGuard` over `navMap`'s pure check.
- [x] **F25 — `components/brand/Logo.tsx`** (TDD) — matches design mirror's `components/brand/Logo.jsx`; OM mark + wordmark, `title`/`tagline`/`size`/`wordmark` props.
- [x] **F26 — `components/icons/TabIcons.tsx`** (TDD) — `TabKey -> icon` selection logic tested as pure mapping; SVG paths lifted from the design mirror's `bv-connect` icon sets (`screens/phone/app.jsx` `ICON.*`, `screens/desktop/dash.jsx` `NAV`).
- [x] **F27 — `components/AppHeader.tsx`** (TDD) — role/scope label derivation extracted as a pure fn + tested; renders OM mark, title, role·scope subtitle. `chrome.hitMin` (44px) touch targets. **Corrected at `/test`:** originally also mounted `RoleSwitcher` here; the design mirror (`bv-connect/screens/desktop/dash.jsx`) puts the switcher only in the sidebar's `side__foot`, so `AppHeader`'s copy was a duplicate — removed (issue #31).
- [x] **F28 — `components/DesktopSidebar.tsx`** (TDD) — tab visibility/selection logic tested as pure fn (same `tabsForRole`-shaped input as Stage 3); renders the `lg`+ left rail nav matching the design mirror, mounts `RoleSwitcher` (stacked) in its footer, matching `side__foot`.
- [x] **F29 — wire into `app/(tabs)/_layout.tsx` + `app/(auth)/sign-in.tsx`** — `useIsDesktop()` switches between the existing phone `Tabs` chrome and `AppHeader`+`DesktopSidebar`. Wiring only, no new unit test (Shared seam). **Scope correction at `/test`:** the plan text called for sign-in to get "the same header treatment above `chrome.maxw`" — the design mirror's own sign-in reference (`MagicLinkLogin.jsx`) has no header at any width, just the capped centered card, so `sign-in.tsx` intentionally got no `AppHeader`/`useIsDesktop` wiring. Verified consistent (same capped-column layout) at 360/768/1024/1440.
- [x] **F30 — unistyles-config-scan regression fix** — `sign-in.tsx`, `app/(tabs)/admin.tsx`, `app/(tabs)/approvals.tsx`, `app/no-role.tsx` were missing the direct `lib/unistyles` import that issue #29 requires (`scripts/check-unistyles-config.js`, tracked-file scan); was failing PR #30's `app-tests` CI job. Fixed (commit `1e4ee4f`) — every Stage 12 file with a module-scope `StyleSheet.create()` carries the same direct import. Re-verified clean at `/test`.

Hand-off: same `/build` → `/test` flow as the rest of this plan; `/test` re-ran the AC#4/#5/#6 role×tab matrix at both a phone width and a `lg`+ desktop width now that F24–F30 have landed (see Stage 12 test results below).
...
- **Design review (playwright-cli vs. Sankalp prototype):** deferred at the first `/test` pass — `lib/theme.ts` was still the generated placeholder. Re-run 2026-07-13 now that real Sankalp tokens are imported (`design/sankalp/design-tokens.json`, `lib/theme.ts` regenerated via `gen:theme`). No `design/sankalp/prototypes/<screen>.html` exists in this repo yet, so the review ran against `USAGE.md`'s objective DoD checklist directly instead of a pixel-match. Found and fixed two real gaps in the screens this item owns (`sign-in.tsx`, `no-role.tsx`, `approvals.tsx`, `admin.tsx`):
  ...
  - **Third finding, root-caused and fixed:** `components/RoleSwitcher.tsx` was initially left unstyled because a styled `StyleSheet.create()` there broke `expo export --platform web` — Expo Router's static-export manifest builder eagerly evaluates `(tabs)/_layout.tsx` (which imports `RoleSwitcher` at module scope) before Unistyles' `StyleSheet.configure()` has run. Filed as [issue #29](https://github.com/mehtamaulik-creator/CMDFW-BalaVihar---Pilot-App/issues/29). **Fixed and closed 2026-07-13**: `RoleSwitcher.tsx` now carries its own direct `import "../lib/unistyles"` side-effect import (not relying on root layout's import running first, since the static-export crawl requires `(tabs)/_layout.tsx` standalone) so `StyleSheet.configure()` is guaranteed to run before its module-scope `StyleSheet.create()` evaluates. `RoleSwitcher.tsx` now ships full theme-token styling (role-colored pill badges, `chrome.hitMin` touch targets) — the header-overlap is gone. Re-verified via `npx expo export --platform web`: clean, no crash, all 21 static routes export.
- **Full suite re-confirmed green after the fixes:** 208/208 vitest, 148/148 pgTAP, typecheck clean, `expo export --platform web` clean.
...
## Stage 12 test results (2026-07-14)

F24–F30 landed (commit `1e4ee4f`). Re-ran `/test` in full per the hand-off note above.

- **Automated suite, fully green:** `npm run test` → 228/228 vitest passing (24 files, up from 208/18 — Stage 12 added `isDesktopWidth`, `Logo`, `iconNameForTab`, `activeSidebarTab`, `appHeaderSubtitle` unit tests + the `unistyles-config-checks` scan test). `npm run typecheck` → zero errors. `npx supabase db reset` + `npx supabase test db` → 148/148 pgTAP, PASS (Stage 12 is client-only, no new migration — RLS layer unchanged since the 2026-07-13 24/24 adversarial audit, not re-run). `node scripts/check-unistyles-config.js` → clean (F30 regression fix confirmed). `npx expo export --platform web` → clean, all 21 static routes.
- **Design parity gate (playwright-cli, 360/768/1024/1440), against `bv-connect/screens/desktop/dash.jsx` + the design-system DoD:**
  - Sign-in, feed/tabs (phone chrome), and the Approvals/Admin "Coming soon" stubs matched the DoD cleanly at all four breakpoints — no horizontal scroll at any width (`document.documentElement.scrollWidth === clientWidth` confirmed at each), fonts/column-cap carried over correctly from the first `/test` pass.
  - **Found and fixed two real defects in the new desktop nav shell**, both filed as GitHub issues before fixing, both closed after re-verification:
    1. **[Issue #31](https://github.com/mehtamaulik-creator/CMDFW-BalaVihar---Pilot-App/issues/31):** `AppHeader.tsx` and `DesktopSidebar.tsx` each independently mounted `<RoleSwitcher />`, so it rendered twice in desktop mode — once in the header (fine), once in the sidebar's ~256px footer rail, where the pills (with long scope-id UUIDs) overflowed the rail into the content area. The design mirror puts the switcher only in the sidebar's `side__foot`; removed `AppHeader`'s copy. Root cause of the remaining overflow: `RoleSwitcher`'s single-row layout doesn't fit 4 pills in a narrow rail. Fix: gave `RoleSwitcher` a `stacked` prop (row+shrink+ellipsis for the header context, column+stack for the sidebar context) plus `flexShrink`/`maxWidth`/`numberOfLines={1}` so long scope-ids truncate instead of overflowing either way. Verified at 360/768/1024/1440 — no overflow, and a live role switch (multirole account, teacher → bv_coordinator at 1024px) updated the tab set correctly with zero console errors.
    2. **[Issue #32](https://github.com/mehtamaulik-creator/CMDFW-BalaVihar---Pilot-App/issues/32):** found while walking `UAT.md` (below) — `supabase/seed/seed.sql` never inserted a `user_roles` row for any seeded parent account (`parent{i}a`/`parent{i}b`), only a `family_members` row, so `parent1a@bv-seed.test.local` always landed on `/no-role` instead of the documented Parent tab set. Pre-existing gap (not introduced by Stage 12), but it blocked UAT-3/UAT-4 as documented. Fixed: added the missing `user_roles` insert (role='parent', scope_type='org', scope_id=null — same shape as `multirole`'s parent grant) for both guardians per family. Re-verified via `db reset` (148/148 pgTAP still green) + live sign-in as `parent1a`.
- **`UAT.md` walkthrough — all 9 scenarios, Pass:**
  - UAT-1 (magic-link happy path, teacher1): Pass — loading → "check your email" → real Inbucket link → Teacher tab set (Feed/Classes/Attendance/Chat).
  - UAT-2 (sign-in failure + retry): Pass — stopped local Supabase mid-request (`supabase stop`), got a preserved-email error state ("Failed to fetch"), restarted (`supabase start`), retry succeeded.
  - UAT-3 (role-derived nav, parent1a): Pass, after the #32 seed fix — exactly Feed/Attendance/Chat, no Classes/Dashboard/Approvals/Admin.
  - UAT-4 (static chip vs. switcher): Pass — parent1a shows a non-interactive `parent · org · —` chip; multirole (4 roles) shows the interactive switcher; never both, never neither.
  - UAT-5 (live role switch, multirole, desktop width): Pass — switched teacher → bv_coordinator while on `/classes`; tab bar updated to Feed/Chat/Dashboard/Approvals and the URL silently moved to `/feed`, no reload, no errors.
  - UAT-6 (stub screens): Pass — Approvals and Admin both show "Coming soon", not a crash or blank screen.
  - UAT-7 (sign-out): Pass — returns to `/sign-in`; reload doesn't sign back in.
  - UAT-8 (session persists across reload): Pass on web (hard reload while signed in stayed on `/feed`, full state intact). Native app-kill/restart half still unverified — same environment gap disclosed in the first `/test` pass (no simulator/emulator tooling here).
  - UAT-9 (zero-role empty state): Pass — temporarily deleted a real account's `user_roles` row via `docker exec ... psql` (documented method), signed in fresh, got the dedicated "no role assigned yet" screen, not a crash; restored the row afterward.
- **Gate markers re-written:** `.claude/.rls-tests-passed` and `.claude/.tests-passed` recorded against the current migration/source-tree hashes (post RoleSwitcher/AppHeader/seed fixes).

**Verdict: GREEN, ready for `/deploy-staging`.** Same disclosed gap as the first pass (AC#10/UAT-8 native half, environment-caused). Two new defects found during this pass (#31 nav-shell RoleSwitcher duplication/overflow, #32 seed-data parent role gap) were both root-caused, fixed, and re-verified green in this branch — closed, not carried forward.

---

## Design-parity re-validation (2026-07-14, via `/design-system`)

A follow-up, higher-fidelity design review of the running app against the **Chinmaya Mission Design System** project directly (`bv-connect/**` — `PersonaSwitcher.jsx`, `MagicLinkLogin.jsx`, `screens/phone/app.jsx`, `screens/desktop/dash.jsx`/`dash.css`), rather than just the DoD checklist Stage 12's own `/test` pass used. Signed in via real magic-link (local Mailpit) as `multirole@bv-seed.test.local` (4 roles) and `parent1a@bv-seed.test.local` (1 role, static-chip path), screenshotted via `playwright-cli` at 360/768/1024/1440.

**Found and fixed five real parity defects, each filed as a GitHub issue before fixing** (matching the established Stage-12 convention):

1. **[Issue #33](https://github.com/mehtamaulik-creator/CMDFW-BalaVihar---Pilot-App/issues/33), closed:** phone/tablet widths (<1024px) fell back to Expo Router's bare default header (just the route title, e.g. "Feed") — `AppHeader` was only wired for `isDesktop` in `app/(tabs)/_layout.tsx`. The design mirror's phone header (`screens/phone/app.jsx` `.hdr`) always shows the OM mark + "Bala Vihar App" + role/scope subtitle. Fixed: `header: () => <AppHeader showSwitcher={!isDesktop} />` now applies at every width.
2. **[Issue #34](https://github.com/mehtamaulik-creator/CMDFW-BalaVihar---Pilot-App/issues/34), closed — reopens/replaces #31's fix:** `RoleSwitcher.tsx` rendered one always-visible pill per held role (row in header, stacked column in the sidebar footer) — a structurally different shape than the design's `PersonaSwitcher.jsx`, which is a single compact trigger (dot + active role + chevron) that opens a bottom sheet. #31's "stacked + ellipsis" treatment made typical cases survive, but a 4-role account at 360px still collapsed into illegible, sub-44px circles — a real DoD violation, not just cosmetic. Fixed: rebuilt `RoleSwitcher.tsx` around a single trigger + RN `Modal` bottom sheet (role rows with colored dot, label, checkmark on active), calling the same `switchRole`/`refreshSession()` path. Single-role static chip (AC#6) unchanged in shape.
3. **[Issue #35](https://github.com/mehtamaulik-creator/CMDFW-BalaVihar---Pilot-App/issues/35), left open as a scoped follow-up:** the switcher/header showed the raw `scope_id` UUID (e.g. "coordinator · session · cf25818b-7e1…") instead of a resolved "Center · Session · Grade/Class" label per AC#6/the design's `scope: "Session · Brampton"` strings. Partial fix landed: switched to the existing `appHeaderSubtitle()` helper, which drops the raw UUID (now "Coordinator · Session"). Full name resolution needs a new read path (join to `classes`/`sessions`/`centers`) and likely an RLS addition — deliberately left for a separate `/refine` → `/architect` → `/migration` pass rather than added ad hoc here.
4. **[Issue #36](https://github.com/mehtamaulik-creator/CMDFW-BalaVihar---Pilot-App/issues/36), closed:** `sign-in.tsx` and `DesktopSidebar.tsx` called `<Logo>` without `title`/`tagline`, so both fell back to the shared component's generic defaults ("Chinmaya Mission DFW" / "Events & Satsangs" — matching `design/sankalp/components/brand/Logo.jsx`'s own defaults) instead of this app's actual branding. Fixed: both call sites now pass `title="Bala Vihar App"` + the matching tagline, per `MagicLinkLogin.jsx`/`dash.jsx`'s `side__logo`.
5. **[Issue #37](https://github.com/mehtamaulik-creator/CMDFW-BalaVihar---Pilot-App/issues/37), closed:** `theme.chrome.deskw` (1180) was generated but never referenced — the desktop shell stretched full-bleed instead of capping/centering like `dash.css`'s `.shell { max-width: var(--app-deskw); margin: 0 auto }`. Fixed: `app/(tabs)/_layout.tsx` wraps the desktop `<Tabs>` tree in a `View` capped at `theme.chrome.deskw`, centered via `alignSelf: "center"`, canvas-colored gutters either side.

**Re-verification after all fixes:**
- `npm run test` → 228/228 vitest (unchanged — all five fixes are wiring/JSX, no new pure-logic surface beyond reusing existing tested helpers). `npm run typecheck` → zero errors. `npx expo export --platform web` → clean, all 21 static routes.
- `playwright-cli` live pass at 360/768/1024/1440 against real local Supabase + Mailpit magic-link sign-in:
  - `multirole@bv-seed.test.local` (4 roles): header + sidebar both show correct "Bala Vihar App" branding at every width; desktop shell now centered/capped at 1440px; switcher trigger + bottom sheet render cleanly at all four widths (no squished pills); a live switch (teacher → coordinator) re-scoped the tab set with the sheet closing automatically, zero console errors.
  - `parent1a@bv-seed.test.local` (1 role): static, non-interactive "Parent · Org" chip renders correctly, unaffected by the switcher rebuild.
...
Closed the scoped follow-up left open by the design-parity re-validation pass above. Added `resolve_my_scope_labels()`, a `SECURITY DEFINER` RPC keyed on `auth.uid()` (not the JWT's `active_role`/scope claims) that resolves every `user_roles` row the caller owns to a friendly `Center · Session · Class` (teacher) or `Center · Session` (coordinator) label — the JWT-claim-keyed approach the existing `classes`/`sessions`/`centers` RLS policies use couldn't cover this, since `RoleSwitcher`'s bottom sheet lists every held role, not just the active one. `bv_coordinator`/`admin`/`parent`/`student` (`scope_type='org'`) are unaffected — they keep the existing "Org" fallback, since there's no `scope_id` to resolve for them. **Correction (2026-08-21, #76 / PR #77):** that last sentence is now wrong for two of the four roles named. `parent` stopped keeping the "Org" fallback on 2026-07-15 (`20260715030000_resolve_parent_scope_label.sql`) — it resolves to the caller's children's first names via `family_members`, and where that resolves to null `appHeaderSubtitle` falls back to "My Children", not "Org". `student` stops keeping it here (#76, `20260817120000_resolve_student_scope_label.sql`) — it resolves to `Center · Session · Class` from the student's active enrollment, falling back to "Org" only when there is no active enrollment; whether such a student should have portal access at all is the access-lifecycle question tracked in #82, not decided by that migration. Only `bv_coordinator` and `admin` still keep the "Org" fallback unconditionally. The parenthetical is also imprecise: all four are `scope_type='org'`, but for `parent`/`student` the label is resolved from a different key than `scope_id` rather than left unresolved for lack of one.

- **Automated suite:** `npm run test` → 231 passing (up from 228, +3 for `appHeaderSubtitle`'s resolved-label cases). `npm run typecheck` → zero errors. `npx supabase db reset` + `npx supabase test db` → 156/156 pgTAP passing (up from 148/18 files, +8 assertions/1 file for the new `150_scope_label_resolution_rpc.sql`; full suite green, no regressions).
- **RLS-adversarial check** (`rls-adversarial-tester` subagent, scoped to `resolve_my_scope_labels()`): forged JWT claims (incl. an `admin`/`org` escalation attempt and a stolen `scope_id`) never changed the `auth.uid()`-keyed result set; a self-insert attempting to seed a foreign `scope_id` was denied with `42501` (no client write grant on `user_roles`); a dangling `scope_id` (no FK constraint on that column) resolved to `null` with no error; `anon` has no execute grant (`42501`, confirmed via `pg_proc.proacl`); adversarial center/session/class names (SQL-comment markers, quotes, HTML) passed through the `||` string concatenation as inert literal text, no injection, no cross-row bleed. **5/5 PASS, no leak found.**
