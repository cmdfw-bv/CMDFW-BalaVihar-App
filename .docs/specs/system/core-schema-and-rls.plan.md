...
- [x] **Step 4: Known seed-data limitation — flag, don't silently fix**
`get_session_compliance_for_staff`'s window is "the last N scheduled `class_meetings` dates `<= current_date`," but the seed's `class_meetings` calendar runs every Tuesday from `2026-01-11` to `2026-05-24` (~19 dates) while `attendance`/`class_updates` are only seeded for the first 4 Tuesdays (`2026-01-13`–`2026-02-03`). Once real wall-clock time passes `2026-05-24` (true for any `db reset` run from this point forward, since today is `2026-07-24`), the RPC's default 4-date window lands on the *last* 4 Tuesdays before session end — dates with **no** seeded attendance/updates — so every class will show near-`0%`, not the varied mix Step 3 intends. **Resolution: out of scope for this task.** This is a seed-realism gap, not a correctness bug (the pgTAP fixtures in Task 13 are what prove the RPC's math is right); flag it for whoever runs the manual `/test` demo pass to re-seed with dates relative to `current_date` if a realistic mixed demo is needed at that time, per this file's own precedent of documenting a known limitation rather than quietly overbuilding a fix (mirrors ADR-0031's own "residual risk" style entry).
