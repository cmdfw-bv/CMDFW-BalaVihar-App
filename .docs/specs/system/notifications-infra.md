...
**Stage:** `/refine` ✓ → `/architect` ✓ (ADR-0028) → `/design` ✓ → `/plan` ✓ (`docs/superpowers/plans/2026-07-21-notifications-infra.md`) → `/migration` is a no-op (no new tables/columns/RLS — confirmed in Design's "Data & RLS impact") → next is `/build`.
...
- The `class_updates`/announcements table and its "post a class update" trigger — owned by `#21 class-update-and-home-feed` (Teacher persona); this item only makes sure its future call into `push-send` won't require rework.
...
Request body: `{ "message_id": "<uuid>" }` — nothing else. The function is deliberately "thin caller, fat function" (ADR-0028): it re-derives everything else from the DB.

Phases (matching the existing `user-role-grant.ts` phased style):
1. **Auth.** `client.auth.getUser(token)` → `caller_user_id`. 401 if missing/invalid.
2. **Validate.** `message_id` present + UUID-shaped. 422 otherwise.
3. **Load + authorize.** Service-role select of `messages` (id, conversation_id, sender_user_id, mention_targets) by `message_id`. Not found → `200 {status:'noop'}` (best-effort, not an error). **New security check surfaced during design (not explicit in the refine/architect brief — flagging for sign-off):** if `sender_user_id !== caller_user_id`, return `403`. Because `push-send` runs service-role and bypasses RLS entirely, nothing else stops an authenticated-but-unrelated caller from repeatedly triggering dispatch about a message they didn't send (a notification-spam vector against that message's real recipients) — RLS on `messages` itself doesn't apply here since the function never queries through the caller's own session. Without this check, ADR-0028's "never trusts the caller for *who* gets notified" is satisfied but "never trusts the caller to trigger *at all*" is not.
4. **Derive recipients.** Service-role select of `conversation_participants` for the message's `conversation_id`, excluding the sender. Filter per participant (see query below).
5. **Fan out.** Service-role select of `push_subscriptions` for the recipient `user_id`s; `web-push` dispatch per subscription with a PII-free payload derived from `conversations.kind` only (never scope_id, never message body/sender): `class` → "New message in your class chat", `session_staff` → "New message in your session staff chat", `leadership` → "New message in the leadership chat".
6. **Cleanup.** Any subscription whose send fails with `statusCode` 404/410 (permanently gone) is deleted from `push_subscriptions`. Other failures (e.g. transient 5xx) are logged and skipped — no retry queue at POC.
7. **Response.** Always `200 { status: 'dispatched', recipients: N, sent: N, cleaned_up: N }` once past phases 1–3 — matches AC#7 (silent no-op, not an error).
