...
|---|---|---|---|---|---|
| [children-attendance-view](children-attendance-view.md) | POC-core | Parent | Parent | own-children (ADR-0018) | `/build` ✓ → `/test` in progress — client-only, no migration needed. Two review rounds (PR #51): the merge-resolution routing bug (Student fell through to the Teacher roster, tripping a `denied` audit write) fixed in `3116e8f`; the null-embed / error-disclosure / query-guard / in-flight items fixed in the round after. Not `Built ✓` until `/test` signs off — corrected per review Minor #8 ([plan](children-attendance-view.plan.md)) |
