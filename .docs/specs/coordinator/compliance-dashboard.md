...
- `design/sankalp/bv-connect/components/dashboard/ComplianceBar.jsx` — two per class, inside one card per class: **"Attendance submission"** (`value=attendance_rate`, `note` = e.g. "3 of 4 meetings fully marked") and **"Class updates posted"** (`value=update_rate`, `note` = e.g. "3 of 4 meetings updated"). Tone auto-derives (≥85/≥70/<70) — no explicit `status` override needed.
