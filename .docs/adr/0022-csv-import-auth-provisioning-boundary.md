...
- **Audit log:** `audit_log` is not extended for import — `audit_log.action`'s `CHECK (action IN ('read','denied'))` constraint (ADR-0019) is unchanged, since a bulk-provisioning write isn't the minor-record-read event that table exists to cover (see design decision #4 in the spec). A completed import instead emits one PII-free structured log line (`{ event: "csv_import_complete", session, processed_count, admin_user_id }`) from the Netlify runtime, as shipped in `netlify/functions/csv-import.ts`. *(Corrected 2026-07-11 — this bullet previously and incorrectly described an `audit_log` row insert that was never implemented and would have violated the CHECK constraint; the structured-log-line approach was the actual decision all along and is now the explicit precedent [ADR-0026](0026-user-role-approval-tiering-and-audit.md) cites for the same gap in `user-role-approval`.)*
