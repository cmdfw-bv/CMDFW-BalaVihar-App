...
**Date:** 2026-06-19
**Status:** **Superseded as the governing doc — kept as reference.** The `cicd-pipeline` UoW (issue #8) has gone through `/refine` → `/architect` → `/design`; the actual scope now being built lives in [`.docs/specs/system/cicd-pipeline.md`](specs/system/cicd-pipeline.md), governed by [ADR-0024](adr/0024-cicd-github-actions-branch-protection.md) + [ADR-0025](adr/0025-cicd-residency-scan-and-codeowners.md). Most of this doc's thinking was carried forward as-is (§2.1 secret scan, §2.3 pgTAP, §2.4 residency/tracker scan, §2.6 CODEOWNERS, §5's job skeleton). **§2.2/§5's CI-driven `deploy-prod` job was explicitly NOT adopted** — `/deploy-staging`/`/promote` stay human-invoked (ADR-0024/ADR-0025). §2.3's "coverage floor" idea also wasn't adopted (false-positive risk) — still an open item here, not built. Treat the spec/ADRs as authoritative wherever this doc and they disagree.
